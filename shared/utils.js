  ...
  getFunctionMetaData: function (functionName, serverless) {
    ...
    for (var eventsIndex = 0; eventsIndex < events.length; eventsIndex++) {
      ...
      if (bindingTypeIndex < 0) {
        throw "Binding not supported";
      }

      var bindingSettings = bindingsJson[constants.bindings][bindingTypeIndex][constants.settings];
      for (var bindingSettingsIndex = 0; bindingSettingsIndex < bindingSettings.length; bindingSettingsIndex++) {
        bindingSettingsNames.push(bindingSettings[bindingSettingsIndex][constants.name]);
      }
    ...
    for (var bindingSettingsIndex = 0; bindingSettingsIndex < bindingSettings.length; bindingSettingsIndex++) {
      ...
      if (required) {
        ...
        if (defaultValue) {
          ...
          throw "required property " + name + " is missing for binding:" + bindingType;
        }
      }
